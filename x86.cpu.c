  ...
  if (modrm & 0b11100000) {
    ...
    case X86_CPU_REG_AX: { // jmp ax
      X86_CPU_gRegs.CS_PLUS_IP =
          X86_CPU_SEGOFF(X86_CPU_gRegs.DS, X86_CPU_gRegs.EAX.word);
      return TRUE;
    } break;
    case X86_CPU_REG_CX: { // jmp cx
      X86_CPU_gRegs.CS_PLUS_IP =
          X86_CPU_SEGOFF(X86_CPU_gRegs.DS, X86_CPU_gRegs.ECX.word);
      return TRUE;
    } break;
    case X86_CPU_REG_BX: { // jmp bx
      X86_CPU_gRegs.CS_PLUS_IP =
          X86_CPU_SEGOFF(X86_CPU_gRegs.DS, X86_CPU_gRegs.EBX.word);
      return TRUE;
    } break;
    case X86_CPU_REG_DX: { // jmp cx
      X86_CPU_gRegs.CS_PLUS_IP =
          X86_CPU_SEGOFF(X86_CPU_gRegs.DS, X86_CPU_gRegs.EDX.word);
      return TRUE;
    } break;
    default:
      break;
...
X86_CPU_InstructionDef X86_CPU_gInstrMap[] = {
    ...
    [0x8C] = {.disas = "MOV [0x%04X], %s", .execute = X86_MOV_RM16_SREG},
    [0x8E] = {.disas = "MOV %s, %s",
              .num_extra_bytes = 1,
    ...
    [0xC7] = {.disas = "MOV [0x%04X], 0x%04X", .execute = X86_MOV_RM16_IMM16},
    [0xEA] = {.disas = "JMP FAR %s:%s",
              .num_extra_bytes = 4,
